...
- PostgreSQL 16.10 — `wfdb02.vya.digital:5432`, sem SSL (`sslmode=disable`). SOURCE: `chatwoot_db` (key `chat-vya-digital`, read-only). DEST DEV: `chatwoot004_dev1_db` (key `vya-chat-dev`, read-write). DEST PROD: `chatwoot004_db` (key `synchat-vya-digital`, read-write). (001-enterprise-chatwoot-migration)
...
- 001-enterprise-chatwoot-migration (2026-05-24): ARQUITETURA FINAL CONFIRMADA: SOURCE DB=chatwoot_db (key=chat-vya-digital, site: chat.vya.digital, read-only). DEST DEV DB=chatwoot004_dev1_db (key=vya-chat-dev, site: vya-chat-dev.vya.digital, read-write). DEST PROD DB=chatwoot004_db (key=synchat-vya-digital, site: synchat.vya.digital, read-write). _ENV_PRESETS: dev=(chat-vya-digital, vya-chat-dev); prod=(chat-vya-digital, synchat-vya-digital). Guardrail DEV-only bloqueia apenas DEST key=synchat-vya-digital e MIGRATION_ENV=prod (source key chat-vya-digital é legítima em dev e prod).
