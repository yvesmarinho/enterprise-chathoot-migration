# Copilot Rules — Enterprise Chathoot Migration

> Arquivo gerado automaticamente por `scripts/scaffold.py` em 2026-04-09T11:37:54Z
...
| **Descrição** | Migração por merge de dados entre instâncias do Chatwoot com deduplicação por chave de negócio |
| **Domínio** | programming |
...
```
Modo: PROGRAMMING. Projeto: enterprise-chathoot-migration. Perfil: devops-programming.
```
...
│   └── SESSIONS/
├── scripts/               # Automação
├── .tmp/                  # Arquivos temporários e de backup (não versionados e removidos automaticamente no final da sessão)
...
|------|---------|-----------|
| 2026-04-09T11:37:54Z | Scaffold inicial criado | Domínio: programming, Linguagem: python || 2026-04-10 | Estratégia de migração alterada: incremental → merge | Registros sobrepostos identificados entre os dois bancos. Requere: chave de negócio por entidade, política de resolução de conflito, rastreio via `custom_attributes->>src_id`. Ver D3 na constitution. |
---
...
*Gerado por scripts/scaffold.py v1.0.0 | 2026-04-09T11:37:54Z*
