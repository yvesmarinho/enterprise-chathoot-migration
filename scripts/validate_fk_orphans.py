...
def main():
    ...
    try:
            ...
            try:
                ...
                if orphan_count == 0:
                    ...
                    for sample in samples:
                        log.warning(
                            f"     Sample: {source_table}.id={sample['id']}, {fk_column}={sample[fk_column]}"
                        )
                    issues_found.append(
                        {"table": source_table, "column": fk_column, "count": orphan_count}
                    )
        ...
        output_path = (
            Path(".tmp") / f'validate_fk_orphans_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        )
