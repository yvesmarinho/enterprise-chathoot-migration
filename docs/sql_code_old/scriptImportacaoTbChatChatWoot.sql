...
BEGIN
    ...
    FOR var_contact_row IN SELECT * FROM public.contacts_tbchat limit 10
    LOOP
        -- Verificar se o contato já existe na nova tabela
        IF NOT EXISTS (SELECT 1 FROM public.contacts WHERE phone_number = CONCAT('+', var_contact_row.phone)) THEN

            IF var_contact_row.id_empresa = '2' THEN
                var_company_name:= 'Bellegarde';
                -- RAISE NOTICE 'Empresa Bellegarde';
            ELSE
                var_company_name:= 'SmartHair';
            ...
            VALUES (
                ...
                null, 
                CONCAT('+', var_contact_row.phone),
                var_account_id,
                TO_TIMESTAMP(var_contact_row.data_reg, 'YYYY-MM-DD HH24:MI:SS'),
                TO_TIMESTAMP(var_contact_row.data_reg, 'YYYY-MM-DD HH24:MI:SS'),
